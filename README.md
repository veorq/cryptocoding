...
⚠️ TODO: "keypair" and "public key" shouldn't be the same type.

⚠️ TODO: in dynamically typed languages (python, ruby, etc), "encode public key to string" and "encode private key to string" shouldn't have the same method name.
...
⚠️ TODO: "16-byte key" and "16-byte IV" shouldn't be the same type.

### Solution

⚠️ TODO: in dynamically typed languages (python, ruby, etc), distinguish key and IV using different param names.
...
Unfortunately in some languages it's not possible to completely clean secret string data, because strings are non-mutable, let's why it's more preferable to use `bytes` for secrets.
